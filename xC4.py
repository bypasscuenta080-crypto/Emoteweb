...
async def GenJoinClanPacket(clan_id, K, V, region):
    # NOTE: The field numbers and packet type are educated guesses.
    # This may need to be adjusted based on actual game traffic.
    # Action ID 5 was a guess for "Join". Trying 6.
    fields = {
        1: 6,  # Guessed action ID for "Join Clan"
        2: {
            1: int(clan_id)
        }
    }
 
    # Determine packet type based on region, similar to other functions
    if region.lower() == "ind":
        packet = '0514'
    elif region.lower() == "bd":
        packet = "0519"
    else:
        packet = "0515" # Default for NA, etc.
 
    return await GeneRaTePk((await CrEaTe_ProTo(fields)).hex(), packet, K, V)

async def GenAcceptFriendPacket(requester_uid, K, V, region):
    # NOTE: The field numbers and packet type are educated guesses.
    fields = {
        1: 8,  # Guessed action ID for "Accept Friend Request"
        2: {
            1: int(requester_uid)
        }
    }
...
async def GenSendFriendRequestPacket(target_uid, K, V, region):
    # NOTE: The field numbers and packet type are educated guesses.
    fields = {
        1: 3,  # Guessed action ID for "Send Friend Request"
        2: {
            1: int(target_uid)
        }
    }
...
async def GenAcceptClanInvitePacket(clan_id, K, V, region):
    # NOTE: The field numbers and packet type are educated guesses.
    # This is for accepting an invite from the mailbox.
    # Action ID 9 conflicts with "Start Match". Trying 11.
    fields = {
        1: 11,  # Guessed action ID for "Accept Clan Invite"
        2: {
            1: int(clan_id)
        }
    }
